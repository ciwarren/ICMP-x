...
class Session:
	...
	def Set_Mode(self, value):
		...
		if value == 8:
			...
			self.current_packet = sniff(filter=f"icmp and src host {self.sender_addr}",lfilter=lambda x:x.haslayer(IP) and x.haslayer(ICMP) and x.haslayer(Raw) and x[ICMP].type == 0x8 and x[ICMP].code == self.session_id and (x[ICMP].id == 0x1 or x[ICMP].id == 0x2)  , iface = INTERFACE, count=1)[0]
	...
	def Start_Session(self):
		self.capture = AsyncSniffer(filter=f"ip src {self.sender_addr}",lfilter=lambda x:x.haslayer(IP) and x.haslayer(ICMP) and x[ICMP].type==0x8 and x[ICMP].code == self.session_id, stop_filter=lambda x:x[ICMP].id == 0x3, prn= Receive_Message(self), iface = INTERFACE)
		self.capture.start()
...
sniff(filter=f"icmp and src host not 192.168.1.135",lfilter=lambda x:x.haslayer(IP) and x.haslayer(ICMP) and x[ICMP].type == 0x8 and ( x[ICMP].id == 0x1 or x[ICMP].id == 0x2 or x[ICMP].id == 0x8 ) and x[ICMP].code == 0x0 , prn= lambda x:Create_Session(x,session_key), iface= INTERFACE)
